...
WORK="${WORK:-$ROOT/clean-consumer}"

rm -rf "$WORK"
mkdir -p "$WORK"

cp "$ROOT/tests/integration/external_consumer.cu" "$WORK/main.cu"
...
INTERNAL_PATTERN='(^|/)(corex_remote_cuda(_g8c)?|corex_metadata([.]c|[.]h|[.]o)?|g7c_corex_fatbin_runtime|corex_fatbin_runtime|corex_launch_config(_g8c)?|corex_device_info(_g8c)?|corex_device_api(_g8c)?|runtime_server(_g8c)?[.]c)(/|$)'
