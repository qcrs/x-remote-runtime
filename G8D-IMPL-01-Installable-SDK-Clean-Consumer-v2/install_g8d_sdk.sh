...
if [[ ! -f "$HERE/dist/lib/libcorex_remote_cudart.so.1.1.0" ]]; then
...
cp -a "$HERE/dist/lib/libcorex_remote_cudart.so.1.1.0" "$PREFIX/lib/"

ln -s "libcorex_remote_cudart.so.1.1.0" \
    "$PREFIX/lib/libcorex_remote_cudart.so.1"
ln -s "libcorex_remote_cudart.so.1" \
    "$PREFIX/lib/libcorex_remote_cudart.so"

cp -a "$HERE/dist/bin/runtime_server_g8c" \
    "$PREFIX/bin/corex-remote-runtime-server"

cp -a "$HERE/corex_remote_cudart_ext.h" \
    "$PREFIX/include/"

cp -a "$HERE/ABI-SURFACE-v1.1.txt" \
    "$PREFIX/share/corex-remote-cudart/"
