...
#include "g7c_corex_fatbin_runtime.h"
...
/* Gate 8C adds remote device-info wire semantics. */
...
static int connect_server(void)
{
    ...
    addr.sin_port = htons(COREX_REMOTE_SERVER_PORT);

    if (inet_pton(AF_INET, COREX_REMOTE_SERVER_HOST, &addr.sin_addr) != 1) {
        close(fd);
        return -1;
    }
...
#define TAKE64(dst) \
    ...
    if (dto_version != G8C_DEVICE_INFO_DTO_VERSION ||
        returned_device != logical_device)
        goto parse_fail;

    if (pos > resp_len ||
        resp_len - pos < G8C_DEVICE_NAME_BYTES)
        goto parse_fail;

    memcpy(
        info_out->name,
        resp + pos,
        G8C_DEVICE_NAME_BYTES);
    info_out->name[G8C_DEVICE_NAME_BYTES - 1] = '\0';
    pos += G8C_DEVICE_NAME_BYTES;
...
static cudaError_t ensure_runtime(void)
{
    ...
    printf("G6E_REMOTE_SESSION=CONNECTED host=%s port=%d protocol=CRX9 version=%u default_stream=LEGACY\n",
           COREX_REMOTE_SERVER_HOST,
           COREX_REMOTE_SERVER_PORT,
           VERSION);
...
void **__cudaRegisterFatBinary(void *fatCubin)
{
    ...
    G7CCorexLoadableImage image;
    char extraction_error[384];

    int extraction_rc =
        g7c_corex_image_from_compiler_wrapper(
            fatCubin,
            &image,
            extraction_error,
